...
def process_region(args):
    ...
    try:
        ...
        with open(raw_pileup_file, 'w') as stdout_f, subprocess.Popen(cmd, stdout=stdout_f, stderr=subprocess.PIPE, text=True) as proc:
...
def process_bed_regions(bed_file, bam_file, ref_fasta, output_file, temp_dir_path=None, processes=64):
    ...
    # Use ultra-fast merge for temp_pileup_results
    if len(all_successful_files) > 100:
        print(f"  - Large number of files detected ({len(all_successful_files)}), using ultra-fast merge...", file=sys.stderr)
        
        # Import the fast merge function
        try:
            from fast_merge_temp_files import fast_merge_temp_files
            
            # Get temp directory from the first file
            temp_dir = os.path.dirname(all_successful_files[0]) if all_successful_files else "temp_pileup_results"
            
            # Use the fast merge function
            success = fast_merge_temp_files(
                temp_dir=temp_dir,
                output_file=output_file,
                batch_size=500,
                processes=min(8, len(all_successful_files) // 500 + 1)
            )
            
            if success:
                print(f"  - Ultra-fast merge completed successfully.", file=sys.stderr)
            else:
                # Fallback to original method
                print(f"  - Ultra-fast merge failed, falling back to original method...", file=sys.stderr)
                _merge_files_fallback(all_successful_files, output_file, temp_dir)
                
        except ImportError:
            print(f"  - Fast merge not available, using fallback method...", file=sys.stderr)
            _merge_files_fallback(all_successful_files, output_file, temp_dir)
    else:
        # Use simple merge for small numbers of files
        _merge_files_fallback(all_successful_files, output_file, temp_dir)
...
def _merge_files_fallback(all_successful_files, output_file, temp_dir):
    """Fallback merge method for smaller numbers of files"""
    ...
    try:
        ...
        with open(output_file, 'w') as final_out:
            for i in range(0, len(all_successful_files), batch_size):
                ...
                result = subprocess.run(cat_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    print(f"  - Merge completed ({len(all_successful_files)} region files).", file=sys.stderr)
